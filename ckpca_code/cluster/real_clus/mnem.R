...
dataset1 = read.csv(file = "E:\\ckpca_code\\dataset\\mnist\\mnist_test.csv", header = FALSE)
dataset2 = read.csv(file = "E:\\ckpca_code\\dataset\\mnist\\mnist_train.csv", header = FALSE)
...
for (time in 1:times){
  ...
  aa = Mclust(data = x, G = kinds)
  cluster1[, 1] = aa$classification
  
  ## KPCA EM
  cluster2 <- tryCatch({
    aa <- Mclust(data = t(cc), G = kinds)
    aa$classification
  }, error = function(e) {
    message("Mclust error")
    rep(1, n)
  })
  ...
  for (order in 2:repeat1) {
    ...
    cluster_result <- tryCatch({
      ...
      message("Mclust error: using previous classification")
...
write.csv(result, file = "E:/mnem.csv")
